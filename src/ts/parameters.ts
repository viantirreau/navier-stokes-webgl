...
class Mouse {
  ...
  private setPosInPx(pos: number[]): void {
    ...
    const distToPivot = Math.sqrt(
      toPivot[0] * toPivot[0] + toPivot[1] * toPivot[1]
    );
    const maxDist = 16;
  ...
  private setRelative(pos: number[]): number[] {
    const canvasSize = Page.Canvas.getSize();
    return [pos[0] / canvasSize[0], pos[1] / canvasSize[1]];
  }
}
...
function bindControls(fluid: Fluid): void {
  ...
  }
  {
    ...
    const updateFloat = (use: boolean) => {
      fluid.useFloatTextures = use;
    };
  ...
  }
  {
    ...
    const updateIterations = (iterations: number) => {
      fluid.minNbIterations = iterations;
    };
  ...
  }
  {
    ...
    const updateTimestep = (timestep: number) => {
  ...
  }
  {
    ...
    const updateStream = (doStream: boolean) => {
      fluidInfo.stream = doStream;
    };
    Page.Checkbox.addObserver(STREAM_CONTROL_ID, updateStream);
    updateStream(Page.Checkbox.isChecked(STREAM_CONTROL_ID));
  }
  {
    const STREAM_HEIGHT = "source-height-id";
    const updateStreamHeight = (h: number) => {
      fluidInfo.streamHeight = h;
    };
    Page.Range.addObserver(STREAM_HEIGHT, updateStreamHeight);
    updateStreamHeight(Page.Range.getValue(STREAM_HEIGHT));
  }
  {
    const BRUSH_RADIUS_CONTROL_ID = "brush-radius-range-id";
    const updateBrushRadius = (radius: number) => {
    ...
    Page.Canvas.Observers.mouseWheel.push((delta: number) => {
      Page.Range.setValue(
        BRUSH_RADIUS_CONTROL_ID,
        brushInfo.radius + 5 * delta
      );
      updateBrushRadius(Page.Range.getValue(BRUSH_RADIUS_CONTROL_ID));
    });
  }
  {
    ...
    const updateBrushStrength = (strength: number) => {
      brushInfo.strength = strength;
    };
  ...
  }
  {
    ...
    const updateColorIntensity = (intensity: number) => {
      fluid.colorIntensity = intensity;
    };
  ...
  }
  {
    ...
    const updateColor = (display: boolean) => {
      fluid.color = display;
    };
  ...
  }
  {
    ...
    const updateDisplayObstacles = (display: boolean) => {
      displayInfo.obstacles = display;
    };
    Page.Checkbox.addObserver(
      DISPLAY_OBSTACLES_CONTROL_ID,
      updateDisplayObstacles
    );
    updateDisplayObstacles(
      Page.Checkbox.isChecked(DISPLAY_OBSTACLES_CONTROL_ID)
    );
  }
  {
    const BLADE_RADIUS = "blade-radius";
    const updateBladeRadius = (r: number) => {
      bladeInfo.radius = r;
    };
  ...
  }
  {
    ...
    const updateCenterOffset = (c: number) => {
      bladeInfo.centerOffset = c;
    };
  ...
  }
  {
    ...
    const updateBladeCount = (c: number) => {
      bladeInfo.bladeCount = c;
    };
